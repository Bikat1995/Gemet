...
useEffect(()=>{if(!tg.token)return;fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'https://gemet-api.onrender.com')}/bids/history`,{headers:{authorization:`Bearer ${tg.token}`}}).then(r=>r.ok?r.json():null).then(x=>x?.bids&&setBids(x.bids)).catch(()=>{})},[tg.token]);
