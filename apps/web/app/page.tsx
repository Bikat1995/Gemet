...
function Countdown({ until }: { until: string }) {
  ...
  return (
    <span className="mono">
      {String(Math.floor(v / 36e5)).padStart(2, "0")}:
      {String(Math.floor(v / 6e4) % 60).padStart(2, "0")}:
...
export default function Home() {
  const [auctions, setAuctions] = useState<Auction[] | null>(null),
    ...
    [clickedId, setClickedId] = useState<string | null>(null);
  const { lang, setLang, t } = useLanguage(),
    tg = useTelegram(),
    router = useRouter();
  useEffect(() => {
    fetch(`${"https://gemet-api.onrender.com"}/auctions?category=${cat}`)
  ...
  const filteredAuctions = auctions?.filter((a) => a.title.toLowerCase().includes(search.toLowerCase())) || null;
  ...
  return (
    <main className="app-shell mx-auto min-h-screen max-w-md px-4 pb-28 pt-5">
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="tile grid h-9 w-9 place-items-center rounded-lg text-[10px] font-bold text-cyan-300"
          >
            {lang === "en" ? "አማ" : "EN"}
          </button>
          <Link
            href="/notifications"
            className="tile grid h-9 w-9 place-items-center rounded-lg text-slate-300"
          >
      ...
      <section className="relative mt-5 min-h-32 overflow-hidden rounded-2xl bg-[linear-gradient(125deg,#172a85,#005acf_56%,#00b9e6)] p-4">
        <PromoArt />
        <p className="relative m-0 text-[11px] font-semibold text-blue-100">
          {t.featured}
        </p>
        <h1 className="relative mb-1 mt-1 max-w-[58%] text-[21px] font-extrabold leading-tight">
          Our Winners
        </h1>
        <p className="relative mb-3 max-w-[55%] text-[10px] leading-snug text-blue-100">
          See who won the latest auctions.
        </p>
        <Link
          href="/winners"
          className="relative inline-block rounded-lg bg-white/20 px-3 py-1.5 text-[10px] font-semibold"
        >
          {t.details} <Icon name="arrow" size={11} className="ml-1 inline" />
        </Link>
      </section>
      <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
        <button className="chip border-cyan-400/45 text-white">
          <Icon name="sliders" size={14} />
          {t.filters}
        </button>
        {["All", "Electronics", "Vehicles", "Property", "Other"].map((c) => (
          <button
            key={c}
            onClick={() => setCat(c === "All" ? "All" : c.toLowerCase())}
            className={`chip ${cat.toLowerCase() === c.toLowerCase() ? "bg-cyan-500/20 text-cyan-300 border-cyan-500/50" : ""}`}
          >
            {c}
          </button>
        ))}
      </div>
      <div className="mt-3 flex items-center gap-2 rounded-xl border border-white/[.08] bg-[#141923] px-3 py-3 text-[#8F9CAE] focus-within:border-cyan-500/50 focus-within:shadow-[0_0_10px_rgba(34,211,238,0.1)] transition-all">
        ...
        <input
          ...
          className="flex-1 bg-transparent text-[12px] text-white outline-none placeholder:text-[#8F9CAE]"
        />
      ...
      <section className="grid grid-cols-2 gap-3">
        {!filteredAuctions ? (
          <div className="col-span-2 py-10 text-center">
            <div className="h-6 w-6 animate-spin rounded-full border-2 border-cyan-500 border-t-transparent mx-auto"></div>
          </div>
        ) : filteredAuctions.length === 0 ? (
          <p className="col-span-2 py-8 text-center text-xs text-slate-500">
          ...
          filteredAuctions.map((a, i) => (
            ...
            >
              ...
              <div className="px-0.5 pb-1 pt-2">
                <h3 className="truncate text-[13px] font-bold text-[inherit]">
                  {a.title}
                </h3>
                <p className="mb-2 mt-0.5 truncate text-[10px] text-slate-500">
                  {a.description}
                </p>
              ...
              {clickedId === a.id && (
                <div className="absolute inset-0 bg-black/50 backdrop-blur-sm grid place-items-center z-10">
