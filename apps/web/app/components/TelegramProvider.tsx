...
export function TelegramProvider({ children }: { children: React.ReactNode }) {
  const [state, setState] = useState<TelegramState>({ isTelegram: false, initData: '', ready: false });
  ...
  useEffect(() => {
    ...
    if (!initData) {
      setState({ isTelegram: true, initData, user, ready: true });
      return;
    }
    ...
    fetch(`${api}/auth/telegram`, {
      ...
      .catch(() => setState({ isTelegram: true, initData, user, ready: true }));
  }, []);

  // Block access if not registered (no phoneNumber) — but not on admin page
  const isAdminPage = typeof window !== 'undefined' && window.location.pathname.includes('/admin');

  if (state.ready && state.isTelegram && !state.phoneNumber && !isAdminPage) {
    return (
      <main
        className="app-shell min-h-screen grid place-items-center bg-[#0A0D14] px-6 text-center"
        data-theme="dark"
      >
